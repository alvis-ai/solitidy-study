...
contract TimeLock {
    error NotOwnerError();
    error AlreadyQuquedError(bytes32 txId);
    error TimestampNotInRangeError(uint blockTimestamp, uint timestamp);
    error NotQueuedError(bytes32 txId);
    error TimestampNotpassedError(uint blockTimestamp, uint timestamp);
    error TimestampExpiredError(uint blockTimestamp, uint expiredtime);
    ...
    function equeue(
        ...
        if (queued[txId]) {
            revert AlreadyQuquedError(txId);
        }
        if (_timestamp > block.timestamp + MAX_DELAY
    ...
    function execute(
        ...
        // check timestamp; block.timestamp > _timestamp
